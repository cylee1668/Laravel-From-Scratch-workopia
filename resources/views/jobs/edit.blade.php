<x-layout>
    ...
    <div class="bg-white mx-auto p-8 rounded-lg shadow-md w-full md:max-w-3xl">
        ...
        <form method="POST" action="{{route('jobs.update', $job->id)}}"  enctype="multipart/form-data">
            ...
            <x-inputs.text id="title" name="title" type="text" label="Job Title" placeholder="Software Engineer" :value="old('title', $job->title)" />
            ...
            <x-inputs.text id="salary" name="salary" label="Annual Salary" type="number" placeholder="90000" :value="old('salary', $job->salary)"/>
            ...
            <x-inputs.text-area id="benefit" name="benefit" label="Benefits"
                placeholder="Health insurance, 401k, paid time off" :value="old('benefit', $job->benefit)" />

            <x-inputs.text id="tags" name="tags" type="text" label="tags (comma-separated)" :value="old('tags', $job->tags)"
            ...
            <x-inputs.text id="address" name="address" type="text" label="Address" placeholder="123 Main St" :value="old('address', $job->address)"/>

            <x-inputs.text id="city" name="city" type="text" label="City" placeholder="Albany" :value="old('city', $job->city)"/>

            <x-inputs.text id="state" name="state" type="text" label="State" placeholder="NY" :value="old('state', $job->state)"/>

            <x-inputs.text id="zipcode" name="zipcode" type="text" label="zipcode" placeholder="02214" :value="old('zipcode', $job->zipcode)"/>
